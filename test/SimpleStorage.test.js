...
describe("SimpleStorage Security", function () {
    ...
    // 4th test — verifies value cannot be read as modified after failed write
    it("does not update value when called by non-owner", async () => {
        await contract.setValue(10);
        try { await contract.connect(attacker).setValue(999); } catch {}
